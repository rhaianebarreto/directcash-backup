...
export type License={student:string;accountId:string;expires:number|null;temporary?:boolean;paused?:boolean};
...
export async function remoteLicense(env:AppEnv,code:string,accountId:string,activate=false):Promise<License|null>{
  ...
  let result:Response;
  try{result=await fetch(new URL(activate?'/activate':'/heartbeat',url),{method:'POST',redirect:'error',headers:{'Content-Type':'application/json','x-livecash-tool':'directcash'},body:JSON.stringify({app:'directcash',key:code,instagramId:accountId}),signal:AbortSignal.timeout(15000)});}
