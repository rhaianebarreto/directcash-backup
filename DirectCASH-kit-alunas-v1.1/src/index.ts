...
async function handle(req:Request,env:AppEnv,ctx:ExecutionContext):Promise<Response>{
  ...
  if(path==='/api/status'&&req.method==='GET'){
    ...
    return json({licensingPaused:env.LICENSE_ENFORCEMENT!=='enabled',profiles:await publicProfiles(env),license:license?{student:license.student,expires:license.expires,temporary:!!license.temporary,paused:!!license.paused}:null,account:a?{id:a.id,username:a.username,expires:a.expires}:null,configured:!!cfg,appId:cfg?.appId||'',owner:cfg?.owner||'',contact:cfg?.contact||'',verifyToken:cfg?.verifyToken||'',callback:url.origin+'/oauth/callback',webhook:url.origin+'/webhook',privacy:url.origin+'/privacy.html',lastWebhook:last?Number(last.value):null,counts});
  }
